...
class Table():
    ...
    def create_table(self):
        board = np.random.normal(loc=0, scale=20, size=(
            self.row_shape, self.col_shape))
        # board = np.array([[1, 6, -9, -4], [29, 36, -3, -46], [-12, -9, 10, -12]])
        self.board = board
        return board
    ...
    def randomize_board_signs(self, p):
        ...
        selected_row = np.random.choice(
            range(int(self.row_shape)), int(self.row_shape*p), replace=False)
        selected_col = np.random.choice(
            range(int(self.col_shape)), int(self.col_shape*p), replace=False)
